# Arrullos Ecommerce Documentation

## Introduction

Fastest E-commerce built with `React`, `NextJS`, `TypeScript`, `React-Query` and `Tailwind CSS`. Its very easy to use, we used `react-query` for data fetching . You can setup your api endpoint's very easily and your frontend team will love using it.
...
Tech specification for this Ecommerce is given below
...
For getting started with the Ecommerce you have to follow the below procedure. First navigate to the `Arrullos-Frontend` directory.

### Step 1 : Configure your env file

Within the project directory you'll find a `.env.local.template` file just rename it as `.env.local`.

** NOTE : ** This file contain `env values` for local development but when you wanna use this Ecommerce for your needs you need to replace this value with `your own real API endpoint`.

### Step 2 : Running the project

Run below command for getting started with this Ecommerce.

```bash
# on Arrullos-Frontend directory
$ yarn
$ yarn dev # which will running the Ecommerce for development
```
...
#start Ecommerce in production mode
yarn start
```
...
- `/src/components`: This folder contains all the related ui components .
- `/src/containers`: This folder contains all the common sections related components.
- `/src/contexts`: This folder contains all necessary context for this ecommerce . Like `cart, ui` etc.
...
For this Ecommerce we didn't provide any actual rest api integration. We have used `react-query` ~~ hook pattern ~~ and fetched data from public json. You will need to edit those hook to integrate your actual API end point. Please go to `framework/basic-rest/` folder for those hooks.
...
If you want to host the Ecommerce in vercel.com then follow the below procedure

- Navigate to `Arrullos-Frontend`
